...
trait Pipeline extends Logging {
  ...
  protected[this] def runPipelineReturnResults(
    ...
    val workflow = Workflow.forPipeline(targets.toMap, rootOutputReportUrl)
    SingletonIo.json[Workflow].write(workflow, workflowArtifact)
    //val htmlArtifact = createOutputArtifact[FlatArtifact](s"summary/$title-$today.html") //now puts in data directory
